package io.github.jorge.anglercraft.objects.blocks.tileentities;
...
public class TileEntityHookForge extends TileEntity implements ITickable {

    private ItemStackHandler handler = new ItemStackHandler(3);
    private String customName;
    private ItemStack forging = ItemStack.EMPTY;
    ...
    public ITextComponent getDisplayName() {
        return this.hasCustomName() ? new TextComponentString(this.customName)
                : new TextComponentTranslation("container.hook_forge");
    }
    ...
    public void update() {
        ...
        if (this.isBurning()) {
            this.burnTime--;
            BlockHookForge.setState(true, world, pos);
        }
        ...
        if (this.isBurning() || !fuel.isEmpty() && (!this.handler.getStackInSlot(0).isEmpty())) {
            ...
            if (!this.isBurning() && this.canForge()) {
        ...
        if (this.isBurning() && this.canForge() && cookTime > 0) {
            cookTime++;
            if (cookTime == totalCookTime) {
                if (handler.getStackInSlot(2).getCount() > 0) {
                    handler.getStackInSlot(2).grow(3);
                } else {
                    handler.insertItem(2, forging, false);
                }

                forging = ItemStack.EMPTY;
                cookTime = 0;
                return;
            }
        } else {
            if (this.canForge() && this.isBurning()) {
                ItemStack output = HookForgeRecipes.getInstance().getHookForgingResult(input);
                if (!output.isEmpty()) {
                    forging = output;
                    cookTime++;
                    input.shrink(1);
    ...
    public static boolean isBurning(TileEntityHookForge tileEntity) {
    ...
    private boolean canForge() {
        if (((ItemStack) this.handler.getStackInSlot(0)).isEmpty())
            ...
            ItemStack result = HookForgeRecipes.getInstance()
                    .getHookForgingResult((ItemStack) this.handler.getStackInSlot(0));
            if (result.isEmpty())
                return false;
            else {
